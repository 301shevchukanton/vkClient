...
import com.example.podkaifom.vkclient.Adapters.DialogsListViewAdapter;
import com.example.podkaifom.vkclient.Classes.Dialog;
...
public class DialogsActivity extends ListActivity {
    ...
    private void startLoading() {
        ...
        currentRequest  = new VKRequest ("messages.getDialogs", VKParameters.from(VKApiConst.COUNT,"25","preview_length", "50"), VKRequest.HttpMethod.GET);
            ...
            public void onComplete(final VKResponse response) {
                ...
                try {
                    ...
                    for (int i = 0; i<messagesArray.length(); i++ ){
                               dialogs.add(new Dialog(Integer.parseInt(messagesArray.getJSONObject(i).getJSONObject("message").getString("id")),
                                       Long.parseLong(messagesArray.getJSONObject(i).getJSONObject("message").getString("date")),
                                       Integer.parseInt(messagesArray.getJSONObject(i).getJSONObject("message").getString("user_id")),
                                       (messagesArray.getJSONObject(i).getJSONObject("message").getString("read_state").equals("1")),
                                       messagesArray.getJSONObject(i).getJSONObject("message").getString("title"),
                                       messagesArray.getJSONObject(i).getJSONObject("message").getString("body")
                               ));
                              myrequests[i]=VKApi.users().get(VKParameters.from(VKApiConst.USER_IDS, String.valueOf(messagesArray.getJSONObject(i).getJSONObject("message").getString("user_id")), VKApiConst.FIELDS,
